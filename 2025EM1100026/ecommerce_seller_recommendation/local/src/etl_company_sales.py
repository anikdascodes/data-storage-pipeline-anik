...
Pipeline Flow:
- Extract company sales data from CSV
- Clean data (trim, type conversion, deduplicate)
- Apply DQ checks
...
def extract(spark: SparkSession, input_path: str) -> DataFrame:
    ...
    # Define schema for company sales
    schema = T.StructType([
...
def clean_data(df: DataFrame) -> DataFrame:
    ...
    cleaned_df = df.withColumn("item_id", F.trim(F.col("item_id")))
...
def apply_dq_checks(df: DataFrame) -> tuple:
    ...
    valid_df = (
        ...
        .select("item_id", "units_sold", "revenue", "sale_date")
    )

    invalid_df = (
        df_with_checks
        .filter(F.col("dq_failure_reason") != "")
        .withColumn("dataset_name", F.lit("company_sales"))
        .withColumn("quarantine_timestamp", F.current_timestamp())
        .select("dataset_name", "item_id", "units_sold", "revenue", "sale_date",
                "dq_failure_reason", "quarantine_timestamp")
    )
...
def main():
    ...
    try:
        ...
        logging.info("Company Sales ETL Pipeline completed successfully")
